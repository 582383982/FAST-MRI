    ...
    with torch.no_grad():
        with tqdm(total=len(data_loader), postfix=[dict(avg_loss=0)]) as t:
            for iter, data in enumerate(data_loader):
                ...
                mean = mean.unsqueeze(-1).unsqueeze(-1).to(cfg.device)
                std = std.unsqueeze(-1).unsqueeze(-1).to(cfg.device)
...
def visualize(cfg, epoch, model, data_loader, writer):
    def save_image(image, tag):
        image -= image.min()
        image /= image.max()
        grid = torchvision.utils.make_grid(image, nrow=8, pad_value=1)
